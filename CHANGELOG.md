...
the first thing a new reader met was a wall of prose about a screen they had
never seen. It now runs end to end — install, a template slide, the data, the
fields, a preview, the merge, and taking it back — with **eight screenshots of
the real pane**, generated by `scripts/manual-shots.mjs` and committed under
`docs/images`.

The numbers agree across every picture and every paragraph: a deck of 8, a
template on slide 3, three rows, three slides added, eleven in the deck
afterwards. A walkthrough whose screenshots disagree with its text teaches a
bug that is not there, and the reader has no way to know which half to believe.

Two smaller things the pictures forced into the open. The walkthrough now starts
with a template slide carrying **no placeholders at all**, which is what the
pane's own order asks for and the opposite of what the old text said. And it
warns that PowerPoint discards a text box you leave empty — a slide of three
empty boxes is a slide of nothing, and step 3 then has nowhere to put a field.

### Fixed — the manual said merged slides land after your template

They land at the **end of the deck**, wherever the template sits. On an 8-slide
deck with a template on slide 3, the merged slides are 9, 10 and 11 — which is
what the pane's own forecast says (*"3 slides added after slide 8"*) and what
`insertDeck` does, targeting the last slide because an insert with no target
puts the new slides at the FRONT.

"Into this deck, after the template block" is true only for a template that is
already last. Also corrected: the manual said an unmatched placeholder takes the
screen's one orange on the merge step. It does not — the tick keeps it there and
the warning reads in muted grey, deliberately. The orange goes to a preview
card, to the card offering a merge back, and to an unmatched chip on the
**fields** step.

### Fixed — two more promises with nothing behind them

"What happens to the template" marked *move it to the end* and *delete it* as
**Planned.** while the backlog carrying them was empty. They are marked designed
and not scheduled, and the guard that catches this now reads prose bullets as
well as table cells — it was added a change earlier and missed these two, three
sections further down the same file.

Two new guards with it: every picture the manual links to must be committed and
every committed picture must be linked, and every internal link must have a
heading behind it.
