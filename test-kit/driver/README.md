...
| `decks.mjs` | Opens a deck in the round's OneDrive folder by name, by reading each row's item GUID and navigating to the editor URL. Clicking a row is unreliable in four different ways. The GUID is only in Edge's DOM, so ids are cached and reused in browsers that cannot read them. |
| `browser.mjs` | Starts a browser on the persistent profile with CDP open on 9333 and leaves it running. `SSF_CHANNEL=chrome` picks Chrome instead of Edge; give it its own `SSF_PROFILE`, because the profile is what carries the signed-in session. `web-login.mjs` cannot: Playwright owns the browser it launches, so it dies when that script returns. Run this one first. |
